...
public interface ItemFactory {
    ...
    static Optional<ItemStack> buildItem(@NotNull Section section, @NotNull String configPath) {
        try {
            ...
            item.editMeta(meta -> {
                ...
                NamespacedKey key = new NamespacedKey(McEnchantGrind.getInstance(), "mcEnchantGrind");
    ...
    static void setItemsForMenu(@NotNull String path, @NotNull Inventory inventory) {
        try {
            Config config = McEnchantGrind.getInstance().getConfiguration();
            Section section = config.getSection(path);
            ...
            for (String key : section.getRoutesAsStrings(false)) {
                ...
                if (itemOptional.isPresent()) {
                    ...
                    else LoggerUtils.warn("No valid slots found for item at path: " + fullPath);
    ...
    private static List<Integer> getCornerSlots(int size) {
        int rows = size / 9;
        List<Integer> corners = Collections.synchronizedList(new ArrayList<>());
    ...
    private static List<Integer> getCenterSlots(int size) {
        int rows = size / 9;
        List<Integer> centers = Collections.synchronizedList(new ArrayList<>());
    ...
    private static List<Integer> getEdgeSlots(int size) {
        List<Integer> edges = Collections.synchronizedList(new ArrayList<>());
        int rows = size / 9;
